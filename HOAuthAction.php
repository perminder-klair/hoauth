...
class HOAuthAction extends CAction
{
      ...
      else 
      {
        ...
        if($accessCode === 1)
        {
          /*$identity = $this->useYiiUser
            ...
            : new CmsUserIdentity($user->email, null);*/
            $identity=new CmsUserIdentity($user->email, null);
  ...
  protected function processUser($user, $userProfile)
  {
    ...
    if($user->isNewRecord) 
    {
      ...
      //insert profile data
      //Yii::app()->profileData->insertRiderData($user, $userProfile);
  ...
  public function yiiUserCheckAccess($user, $render = true)
  {
    ...
    {
      $error = CmsUserIdentity::ERROR_STATUS_NOTACTIV;
      $return = 2;
    }
    else if($user->status==-1)
    {
      $error = CmsUserIdentity::ERROR_STATUS_BAN;
      $return = 0;
    }
    else 
    {
      $error = CmsUserIdentity::ERROR_NONE;
      $return = 1;
    }
